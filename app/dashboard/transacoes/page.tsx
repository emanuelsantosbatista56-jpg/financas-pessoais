...
export default function TransacoesPage() {
  ...
  const filtradas = transacoes
    ...
    .sort((a, b) => {
      ...
      if (ordenacao === 'valor_asc') return Number(a.amount) - Number(b.amount)
      return 0
    })
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Registre e filtre receitas e despesas</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 ${
              filtrosAbertos || temFiltrosAtivos ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'
            }`}
          >
            ⚙️ Filtros
          ...
          {(['todos', 'income', 'expense'] as const).map((f) => (
            <button key={f} onClick={() => setFiltroTipo(f)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                filtroTipo === f ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'
              }`}>
        ...
        {carregando ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              {temFiltrosAtivos ? 'Nenhuma transação encontrada com estes filtros' : 'Nenhuma transação ainda'}
            </p>
            {temFiltrosAtivos ? (
              <button onClick={limparFiltros} className="mt-3 text-indigo-400 hover:text-indigo-300 text-sm underline">
                Limpar filtros
              </button>
            ) : (
              ...
              <div key={t.id} className="flex items-center justify-between px-5 py-4 hover:bg-gray-800/50 transition-colors group">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {t.categories && (
                        <span className="text-xs px-2 py-0.5 rounded-full" style={{
                          backgroundColor: t.categories.color + '30', color: t.categories.color
                        }}>
                          {t.categories.name}
                        </span>
                      )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => excluir(t.id)}
                    className="text-gray-700 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-sm">✕</button>
                </div>
              </div>
            ))}
          </div>
